...
void updated_twist_callback(const geometry_msgs::Twist& twist_cmd)
{
    ...
    if(twist_cmd.linear.x == 0 && twist_cmd.linear.y == 0) 
    {
...
void loop()
{
  // The t.update() call is removed
  loop_counter += 1;
  
  // Publish the motors' current states and check for new ROS messages
  // This runs at approx. 25Hz (10ms delay + processing time)
  if(loop_counter >= 4) { // Roughly 25Hz
    noInterrupts();
    wheel_ctr.wheel_vel_L = (motor_left_counter - motor_left_old) * encoder2linear_v;
    wheel_ctr.wheel_vel_R = (motor_right_counter - motor_right_old) * encoder2linear_v;
