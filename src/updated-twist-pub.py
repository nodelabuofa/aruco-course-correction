...
class UpdatedTwistPub:
    def __init__(self):
        ...
        self.desired_corners = {
            0: {'u': -100.0, 'v': -50.0},  # Target for corner 0
            1: {'u': 100.0, 'v': -50.0},   # Target for corner 1
            2: {'u': 100.0, 'v': 50.0},    # Target for corner 2
            3: {'u': -100.0, 'v': 50.0}    # Target for corner 3
        }
    ...
    def interaction_matrix_callback(self, aruco_corners_msg):
        ...
        interaction_matrices = [] # list to hold 4 vertically stacked 2x6 interaction matrices
        error_vectors = [] # list to hold 4 stacked 2x1 error vectors
        ...
        for i in range(4): # 0, 1, 2, 3
            ...
            v_flipped = 360 - v_0 # flip origin from top left to bottom left to follow math conventions (360 is pixel resolution height)
            ...
            rospy.loginfo(f'Corner {i}: Current (u,v)=({u_0},{v_0}), Desired (u,v)=({u_desired},{v_desired})')
        ...
        v_twist = -lambda_gain * L_stacked_inv @ e_stacked  # Shape: (6,1)
